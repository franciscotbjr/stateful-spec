# Intake — raw inbound work (this repo's instance)

> The **raw inbox** for work that has not yet been triaged into a numbered opportunity. The
> canonical, triaged backlog stays in [`../backlog.md`](../backlog.md); the roadmap stays in
> [`../memory.md`](../memory.md).
>
> This README documents **this project's** intake folders and the READY convention only. The
> methodology canon lives in `methodology/backlog.md` (intake + backlog pipeline) and
> `methodology/qa-phase.md` (post-delivery QA, routed by type) — at the **repo root**, since this
> repo IS the methodology source. Do not duplicate that canon here.
...
**promoted** into [`../backlog.md`](../backlog.md) as a stable `O-NNN` entry (then possibly onto the
