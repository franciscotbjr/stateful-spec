...
- [ ] 021 — Fix stale `history/.archived/memory.md` archiving (O-008) → `history/021-fix-archived-memory.md`
- _(context)_ iteration 020 (O-007 `rust-design-system`) merged to `main` via PR #39 (`d5971a3`); close-session on `chore/020-end-session` (`0e61ce0`), pending PR.
...
- Currently open: 021 — Fix stale `history/.archived/memory.md` archiving (O-008) → `history/021-fix-archived-memory.md`
...
|---|------|------|--------|------|
| 021 | fix-archived-memory | bugfix | in-progress | `history/021-fix-archived-memory.md` |
...
> merges into the Archive row at the bottom — first appending that row's full
> content verbatim to `history/.archived/memory.md` so the fold loses no detail
> (see `methodology/history-archiving.md`).
