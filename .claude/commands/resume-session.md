...
- Triage the intake inbox at the kickoff moment
...
2. **Triage the intake inbox** (kickoff triage) — same as `start-session` STEP 1.5: scan `.stateful-spec/intake/{Backlog,Discovery,QA}/` for `ready` items, route promote→`O-NNN`/discard per `methodology/backlog.md`, mark each routed item `triaged`.
3. If there is **no** in-progress iteration that matches this task, **create one** before substantive edits:
...
4. **`.stateful-spec/history/`** — List the directory to identify existing iterations. **Do not read all history files**, and **do not list or bulk-read `history/.archived/`** (the cold store). The **Engramas** section in `memory.md` provides compiled summaries of each iteration. Only read a specific iteration file if the Engramas section is insufficient — open it by the exact path in the History Index `File` column (which may point into `history/.archived/`). When determining the next `NNN`, scan **both** `history/` and `history/.archived/` so an archived number is never reused. See `methodology/history-archiving.md`.
...
### STEP 2.5 — Triage the Intake Inbox (kickoff triage)

Run the kickoff triage before asking what's next, same as `start-session` STEP 1.5: scan
`.stateful-spec/intake/{Backlog,Discovery,QA}/`, route `ready` items promote→`O-NNN`/discard per
`methodology/backlog.md`, mark each routed item `triaged`. Surface the `ready` items in STEP 3 as
candidate work. If none, continue.
