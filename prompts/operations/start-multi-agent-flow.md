---
description: Start or join an autonomous multi-agent milestone flow in a given role (pm|engineer|architect)
---

# Start Multi-Agent Flow

> **Applies to: software.** Starts or joins the optional autonomous multi-agent flow (two-agent or three-agent) defined
...
**Usage:** `start-multi-agent-flow pm` · `start-multi-agent-flow engineer` · `start-multi-agent-flow architect` *(three-agent flow)*
...
Read the argument: it must be `pm` (Agent 1 — PM/Architect-planner), `engineer` (Agent 2 — Senior
Engineer), or `architect` (Agent 3 — read-only spec reviewer; **three-agent flow only**). If it is
missing or invalid, ask which role to assume and stop until answered.
...
> **Optional flow tool — ask permission first.** Published reference implementations of the
> coordination verbs exist (`stateful-spec-flow` / `@stateful-spec/flow`; source in the methodology
> repository's `packages/`). They are **not required** — you may relay turns by hand or use a
> scheduler/loop. If you want to use one, **tell the user it is available and ask permission before
> using it** (per `methodology/roles.md`); otherwise proceed without it.
...
- **No active flow** (`flow-state.md` absent, or `process_status: DONE`): **kick off.**
  ...
  2. Run **Analyze + Plan** (`methodology/phases/01-analyze.md`, `02-plan.md`). Produce the
     **master plan**: a milestone breakdown M1…Mn. This is the umbrella iteration — create
     ...
     `start-session`).
  3. **Choose the flow mode** — **two-agent** (PM + Engineer; default) or **three-agent** (adds a
     read-only Architect spec reviewer — a design gate before each implementation; use it for
     design-sensitive work). Create `.stateful-spec/flow-state.md` from
     `templates/implementation/flow-state.md`: `process_status: PLANNING`, `iteration: NNN-<name>`,
     `turn: pm` (three-agent flow: also record each milestone's `profile`). After the plan is
...
**If role = `architect` (three-agent flow):**

- Require an existing `flow-state.md`. If none exists, tell the human the PM must kick off first
  (`start-multi-agent-flow pm`) and stop.
- Otherwise **attach** as the **read-only** reviewer — do **not** create a new iteration or session,
  and **never edit code or run the build**. You act only on `turn: architect` / `step: ARCH_REVIEW`.
  Go to STEP 3.
...
3. If `turn` ≠ your role → **stand by on a blocking poll** that returns the instant the turn flips
   (the primary standby); a fixed-interval wake (a scheduler, the `/loop` skill) is a fallback only.
   On a poll timeout, run the liveness / spawn-death watchdog and re-poll.
4. If `turn` == your role → perform STEP 4 for the current `step`, then re-enter the blocking poll to
...
After acting, always: update `flow-state.md` frontmatter (`turn`, `step`, the round counter
(`review_round` / `spec_review_round`) and `profile` if applicable, `updated_by`, `updated_at`, and
`seq`), append a turn-log line (`timestamp · role · action · file`), and append a Session Log entry
to the umbrella iteration file.
...
**Three-agent flow — design gate (additions).** In the three-agent flow the PM does **not** hand
straight to the engineer at `SPEC_PENDING`; the spec passes the architect first:

- **PM `SPEC_PENDING` / `SPEC_CHANGES_REQUESTED`** — After writing (or revising) the milestone spec,
  **submit it to the reviewer**: `step: ARCH_REVIEW`, `turn: architect`, and **spawn the architect**
  (born reading the draft spec). Then blocking-poll.
- **Architect `ARCH_REVIEW`** — Review the spec **read-only** against the known failure classes
  (never build, never edit). End your final message with a parseable verdict block (scaffold
  `templates/implementation/arch-verdict.md`). The PM recovers the verdict and runs the matching
  transition (the verbs are turn-gated, not identity-gated):
  - approve → `step: SPEC_APPROVED`, `turn: pm`.
  - request changes → `step: SPEC_CHANGES_REQUESTED`, `turn: pm`, `spec_review_round += 1`. If
    `spec_review_round > max_spec_review_rounds` → `BLOCKED` (design-gate stall).
- **PM `SPEC_APPROVED`** — Hand off to the engineer: `step: SPEC_READY`, `turn: engineer`, and spawn
  the engineer for the milestone's `profile`.
- **Engineer hand-back** — If the milestone is mis-scoped for its profile, return it:
  `step: SPEC_PENDING`, `turn: pm` (un-counted); the PM re-scopes / re-profiles and the architect
  re-reviews.
